...
- **广泛支持**：目前 EASI([v0.2.2](https://github.com/EvolvingLMMs-Lab/EASI/releases/tag/0.2.2))支持 **26 个空间智能模型**和 **27 个空间基准测试**。
- **双后端支持**：
  - **VLMEvalKit**：丰富的模型库，内置评判功能。
  - **lmms-eval**：轻量级、基于 accelerate 的分布式评测。
...
- **统一 CLI 支持**：新增 EASI-8 一键评测流程，兼容 VLMEvalKit 和 lmms-eval 后端，并支持自动收集结果和可选排行榜提交。
...
安装脚本会自动安装两种评测后端（VLMEvalKit 和 lmms-eval）及固定版本依赖：
